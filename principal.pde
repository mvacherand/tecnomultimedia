 ...
 //decalro las instancias de mis clases(objetos)
  ...
  void dibujar() {
    ...
    case 0:
      ...
      break;
    case 1:                            // en el caso/ estado 1 se dibuja el fondo escenario, el personaje, los obstáculos, aparece el tiempo,vidas
      background(0);
      fondo1.dibujarPantalla();
      fill(#FF4800);
      image(vida,13,20,90,40);
      //text("VIDAS: ",10,50);
      text(vidas,105,50);
      ...
      for (int i = 0; i<cantObstaculos; i++) {
        obstaculos[i].dibujarObstaculo( 3);
  ...
  void keyPressed() {               // detecta cuando el persoanje salta 
    if (key == ' ' && personaje.saltando == false) {
